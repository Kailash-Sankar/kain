...
router.post('/', async (ctx) => {
    ...
    await page.goto(`${process.env.UI_URL}/${type}?name=${name}&range=${range}`, { waitUntil: 'networkidle2' });
    await page.waitForSelector('#puppeteer', { visible: true })
    await page.screenshot({ path: `public/images/screenshot/${name}.png`, fullPage: true });
    await browser.close();
